	...
	/* 	}); */
		...
		$(document).ready(function() {
		    $('#test02').on('input', function() { 
		    	// (X) a) 테스트02라는 아이디를 가진 곳에, b) function을, c) input해라
		    	// (O) a) 테스트02라는 아이디를 가진 곳에 , c) 'input'이라는 <이벤트>가 발생한다면, b) function을 호출해라
		    	// -> event driven 이벤트 중심? 기반? 프로그래밍
		    	// c) 'input' 이벤트, 즉 어떤 내용이 변경된다면..
		    	// c-2) 즉, 이벤트명은 '글자'다. 고로 이벤트명을 우리는 외워야 한다.
		    	// c-3) 그런데 망할, jquery랑 일반 javascript(=WEB API)는 이벤트명이 쬐깐 다르다 (망할)		    	
...
<div id="root">
			...
			<section id="container">
				<form role="form" name="form" method="post" action="/board/write" enctype="multipart/form-data">
					<input type="hidden" id="parent_bno" name="parent_bno" value="0">
					<input type="hidden" id="depth" name="depth" value="1">
					<div align="center" class="main">
					
					<table>
						<tbody id="tbody_fileAdd_btn">
							<tr>
								<td>
								...
								<td>
									...
									<br/>
									<textarea id="test" name="content" class="chk" placeholder="내용을 입력하세요. 필수 입력 부분입니다." maxlength=4001 style="width:800px;" ></textarea>
